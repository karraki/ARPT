...
;-
function checkpar,param,goodtype,goodrank, $
            CALLER=caller, DEFAULT=default, DIMEN=dimen, $
            NPTS=npts, RANK=rank, TYPE=type, silent=silent, errstr=errstr

   errval = 0
   errstr = ''   ; initialize error string

   ; Not enough inputs
   if n_elements(goodtype) eq 0 then begin
     print,'Syntax - error = checkpar(param,goodtype,goodrank, CALLER=caller, DEFAULT=default,'
     print,'                          DIMEN=dimen, NPTS=npts, RANK=rank, TYPE=type, silent=silent'
   ...
   ; Check variable rank
   if type ne 0 and n_elements(goodrank) gt 0 then begin
